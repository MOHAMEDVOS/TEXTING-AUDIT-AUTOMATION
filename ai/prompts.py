...
- RELATIVE IS REALTOR/AGENT: If the contact says a family member (wife, husband, parent, sibling, etc.) is a realtor, agent, or broker, label "Do Not Call" for the owner — we do not compete with their rep. This is valid DNC, not "Listed" or "Not Interested" (e.g. "My wife is a realtor").
- MINOR / KID OWNER: If the contact or number holder is a child/minor (e.g. "I'm a kid", "this is my son's phone", "he is only 12", under 18), label "Do Not Call" — not Wrong Number alone. We must not solicit minors.
...
F6 "Agreed to call without pre-qualifying." (ONLY when the CONTACT explicitly agrees to a scheduled/confirmed call — e.g. "call me tomorrow", "yes you can call" — AND the lead supplied zero pillars before that point. Do NOT fire when the agent merely OFFERS a callback: "I can give you a call later", "Can my partner call you?", "unavailable but I'll call later" — especially when the same message asks a qualifying question like "any plans to sell?". Do NOT fire on agent-only "I'll call you" without prior contact call agreement.).
