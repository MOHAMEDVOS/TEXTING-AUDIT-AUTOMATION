...
from database.trend_counts import recompute_trend_counts
...
async def _recompute_trend_counts(conn, agent_name: str, audit_date=None) -> None:
    """Recompute one agent's trend snapshot counters from live conversation_scores.

    The query itself lives in database/trend_counts.py so the repair pass runs
    the identical thing. See that module for why every counter is gated on
    validation_log, why the date match uses the convo_date-preferring CASE, and
    why each subquery scopes to ts.account_email and dedups by contact_id.
    """
    await recompute_trend_counts(conn, agent_name, audit_date)
...
async def _save_trend_snapshot(agent_name: str) -> None:
    ...
    try:
        async with app.state.pool.acquire() as conn:
            ...
            # conversations_analyzed is the only counter written from the audit
            # details. total_issues / late_response_flags / wrong_label_flags
            # are the validation-gated ones and belong to
            # database/trend_counts.py, which this function calls once the row
            # exists — this used to re-derive them here by matching validation
            # rows on contact NAME, a copy of logic app.py's canonical query
            # dropped in migration 009 for exactly the reason recorded there.
            # A freshly-completed audit has nothing validated, so the row is
            # inserted at zero and climbs as the auditor clicks Valid.
            conversations_analyzed = 0
            try:
                details = score_row["details"] or {}
                if isinstance(details, str):
                    details = json.loads(details)
                conversations_analyzed = len(details.get("per_conversation", []))
            except Exception as _e:
                logger.debug("swallowed: %r", _e)
            ...
            await conn.execute(
                ...
                0,   # total_issues — recomputed below
                ...
                0,   # late_response_flags — recomputed below
                0,   # wrong_label_flags   — recomputed below
            )
            # Fold in whatever the auditor has already validated for this day
            # (normally nothing on a fresh run; non-zero when an audit is
            # re-run for a day someone has already worked through).
            await _recompute_trend_counts(conn, snapshot_agent_name, audit_date_val)
